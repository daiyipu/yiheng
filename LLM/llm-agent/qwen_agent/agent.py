...
class Agent(ABC):
    ...
    def __init__(self,
        ...
        if isinstance(llm, dict):
            self.llm = get_chat_model(llm)      # 实例化LLM
        else:
            self.llm = llm
        ...
        if function_list:
            for tool in function_list:
                self._init_tool(tool)       # 初始化工具

        self.system_message = system_message    # agent信息
        self.name = name
    ...
    def run(self, messages: List[Union[Dict, Message]],
        ...
        for msg in messages:
            if isinstance(msg, dict):
                new_messages.append(Message(**msg))     # Message()类实现了对输入更方便的校验等管理
            else:
