...
def run_main():
    ...
    red_var_percentage_pca,red_var_accumulation_percentage_pca,red_feature_name = red_pca.PCA_Reduction()
    white_var_percentage_pca, white_var_accumulation_percentage_pca,white_feature_name = white_pca.PCA_Reduction()
    ...
    for i in range(size-1):
        for j in range(i+1,size):
            plt.scatter(Red_WineQuality_Data_Reduction_PCA[:, i], Red_WineQuality_Data_Reduction_PCA[:, j],s=5)
            plt.xlabel("主成分"+str(i+1))
            plt.ylabel("主成分"+str(j+1))
            plt.grid(True)
            plt.savefig("./红葡萄酒数据集降维可视化PCA/" + str(i) + "-" + str(j) + ".jpg", bbox_inches='tight')
            plt.close()
    ...
    for i in range(size-1):
        for j in range(i+1,size):
            plt.scatter(White_WineQuality_Data_Reduction_PCA[:, i], White_WineQuality_Data_Reduction_PCA[:, j],s=5)
            plt.xlabel("主成分"+str(i+1))
            plt.ylabel("主成分"+str(j+1))
            plt.grid(True)
            plt.savefig("./白葡萄酒数据集降维可视化PCA/" + str(i) + "-" + str(j) + ".jpg", bbox_inches='tight')
            plt.close()
